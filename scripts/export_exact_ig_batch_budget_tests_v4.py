...
class CharacterTokenizer:
    ...
    def __call__(self, text, **kwargs):
        if not kwargs.get("return_offsets_mapping"):
            raise AssertionError("V4 target tokenization requires offsets")
        return {
...
def _policy() -> ExactIGPrecisionPolicy:
    return ExactIGPrecisionPolicy(
        mode="cpu_fp32_exact_ig_semantics",
...
def main() -> None:
    ...
    payload = {
        "schema": "exact_ig_fp32_batch_budget_tests_v4",
        "logits_element_size": 4,
        "rows": rows,
        "all_pass": all(row["pass"] for row in rows),
        "gate_pass": all(row["pass"] for row in rows),
        "optimizer_steps": 0,
        "scheduler_steps": 0,
        "checkpoint_writes": 0,
    }
