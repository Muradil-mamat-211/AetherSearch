...
def _allclose_rows(
    ...
    for left_row, right_row in zip(left, right, strict=True):
        ...
        passed = passed and bool(
            torch.allclose(left_tensor, right_tensor, rtol=RTOL, atol=ATOL)
        )
...
def _ranking(values: Any) -> tuple[int, ...]:
    return tuple(
        sorted(
            range(len(values)),
...
def main() -> None:
    ...
    try:
        ...
        policy = production_precision_policy(PRODUCTION_PRECISION_MODE)
        ...
        maximum_phi_difference = _maximum_difference(
            fast.score_by_prefix,
            sequential.score_by_prefix,
        )
        maximum_ig_difference = _maximum_difference(
            fast.immediate_ig,
            sequential.immediate_ig,
        )
        sign_agreement = all(
        ...
        ranking_agreement = (
            ...
            == _ranking(sequential.immediate_ig)
        )
        maximum_telescoping = max(
            abs(float(fast.telescoping_error)),
            abs(float(sequential.telescoping_error)),
        )
        dtype_gate = all(
            ...
            for metadata in (
                fast.runtime_metadata,
                sequential.runtime_metadata,
            )
        )
        numerical_gate = bool(
            token_allclose
            and max_abs_diff <= MAX_TOKEN_ABS_DIFF
        ...
        local_result = {
            ...
            "max_abs_phi_diff": maximum_phi_difference,
            "max_abs_ig_diff": maximum_ig_difference,
            ...
            "sequential_runtime_metadata": dict(
                sequential.runtime_metadata
            ),
    ...
    gate_tensor = torch.zeros(1, dtype=torch.int32, device=device)
    if rank == 0:
        ...
        payload = {
            "schema": "exact_ig_fp32_fsdp4_parity_v4",
            ...
            "official_fast_sequential_allclose": all(
                row.get("numerical_gate_pass") is True for row in rows
            ),
            "maximum_token_log_prob_abs_diff": max(
                (
                    float(row.get("max_abs_token_log_prob_diff", float("inf")))
                    for row in rows
                ),
                default=float("inf"),
            ),
            "maximum_phi_abs_diff": max(
                (
                    float(row.get("max_abs_phi_diff", float("inf")))
                    for row in rows
                ),
                default=float("inf"),
            ),
            "maximum_ig_abs_diff": max(
                (
                    float(row.get("max_abs_ig_diff", float("inf")))
                    for row in rows
                ),
                default=float("inf"),
            ),
        ...
        payload["gate_pass"] = bool(
            payload["fsdp_window_restore_pass"]
            and payload["official_fast_sequential_allclose"]
        )
        gate_tensor.fill_(int(payload["gate_pass"]))
        args.output.parent.mkdir(parents=True, exist_ok=True)
    ...
    if local_result.get("error") is not None or not bool(gate_tensor.item()):
        raise SystemExit(2)
